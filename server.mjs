...
await Promise.all([mkdir(PROJECTS, { recursive: true }), mkdir(DATA, { recursive: true })]);

const accessKeyPath = path.join(DATA, "access-key");
let accessKey = process.env.KIDDO_ACCESS_KEY || "";
if (!accessKey) {
  try { accessKey = (await readFile(accessKeyPath, "utf8")).trim(); }
  catch {
    accessKey = randomBytes(12).toString("base64url");
    await writeFile(accessKeyPath, `${accessKey}\n`, { mode: 0o600 });
  }
}
...
function authorized(req, url) {
  const supplied = req.headers["x-kiddo-key"] || url.searchParams.get("key");
...
7. Before finishing, run: node ../../scripts/validate-project.mjs app.html
...
async function runCodex(dir, prompt) {
  const args = ["exec", "--json", "--sandbox", "workspace-write", "--skip-git-repo-check", "--ignore-user-config", "-C", dir, prompt];
  return await new Promise((resolve, reject) => {
    ...
    child.on("close", (code, signal) => {
      ...
      else resolve(finalText || "I updated your app! Give it a try, then tell me what you want to change.");
    });
  });
}

async function handleApi(req, res, url) {
  if (!authorized(req, url)) return json(res, 401, { error: "Please use the private link shown on the Raspberry Pi." });
  ...
  if (req.method === "POST" && url.pathname === "/api/projects") {
    ...
    return json(res, 201, await readProject(id));
  }
  ...
  if (req.method === "POST" && action === "chat") {
    ...
    try {
      ...
      let reply = await runCodex(dir, buildAgentPrompt(project, message));
      let html = await readFile(appFile, "utf8").catch(() => "");
      let validation = validateHtml(html);
      if (!validation.ok) {
        await writeFile(appFile, oldHtml);
        html = oldHtml;
        reply = "I had a little wobble, so I kept your working app safe. Try asking me for that change one more time!";
        validation = { ok: true, errors: [], warnings: ["The attempted update did not pass checks; the previous version was restored."] };
      }
      ...
      return json(res, 200, { html, message: buddyEntry, validation });
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    json(res, error.code === "ENOENT" ? 404 : 500, { error: error.message || "Something went wrong." });
  }
});

server.listen(PORT, HOST, () => {
  console.log(`Kiddo Programmer is ready.`);
  console.log(`On this Pi: http://localhost:${PORT}/?key=${accessKey}`);
  console.log(`On the iPad: http://<PI-IP>:${PORT}/?key=${accessKey}`);
});
