...
Do not edit, create, or delete any files. Review only app.html in the current folder.

Your review must:
1. Run: node ${JSON.stringify(validatorPath)} app.html
...
6. Avoid endless polish requests. Use "improve" only for a concrete defect, missed requirement, safety issue, or meaningful usability problem.
7. Act as the version gatekeeper. Supply a short commitMessage (72 characters or fewer) describing the finished app change. Do not copy the child's message or include personal information. The trusted server will run Git only when your verdict is "pass".
...
export function buildRevisionPrompt({ age, projectName, kidMessage, feedback, validatorPath }) {
...
Read the current app.html and address every concrete issue in the supervisor feedback.

Rules:
1. Edit ONLY app.html in the current project folder. Never inspect parent folders, credentials, configuration, or other projects.
...
5. Run: node ${JSON.stringify(validatorPath)} app.html
